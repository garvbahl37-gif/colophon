...
|---|---|---|---|
| vector only | **0.78** | **0.907** | 22 |
| lexical only | 0.61 | 0.825 | 9 |
| hybrid | 0.74 | 0.895 | 10 |
| hybrid + rerank | 0.74 | 0.864 | 211 |
| full pipeline (shipped) | 0.74 | 0.867 | 13362 |

Reranking lowers nDCG here and costs hit@3, and it is not the noise floor doing
it — setting `RETRIEVAL_MIN_RERANK_SCORE=0` reproduces the result exactly. The
local cross-encoder reorders these questions worse than RRF fusion already had.

Two things that measurement does not establish. Production reranks with
`llm:listwise`, not this model, so it is untested by this run. And 40 passages is
still small: the shipped pipeline loses 4 of 23 questions to the baseline, and
reading them at `/evals` shows why — `gw-vs-client-retries` retrieves the HTTP
client library's "Client Backoff" instead of the gateway's "Retry Policy", which
is precisely the confusion the distractors were written to cause. That is a real
failure worth fixing, not a scoring artefact.
