...
for await (const chunk of stream as unknown as AsyncIterable<Record<string, any>>) {
  ...
  if (chunk.type === "data-citations") {
    ...
    for (const c of chunk.data) console.log(`  [${c.marker}] ${c.documentTitle} ${c.headingPath.join(" > ")}  score=${c.score.toFixed(3)}`);
  }
  if (chunk.type === "data-grounding") {
    const g = chunk.data;
    console.log(`\n\x1b[36m── grounding ──\x1b[0m supported=${g.supported} density=${(g.citationDensity*100).toFixed(0)}% issues=${g.issues.length}`);
